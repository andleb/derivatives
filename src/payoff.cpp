...
PayoffCall::PayoffCall(double p_strike)
    : m_strike(p_strike)
{}
...
double PayoffCall::operator()(double p_spot) const
{
    return std::max<double>(p_spot - m_strike, 0);
}

PayoffPut::PayoffPut(double p_strike)
    : m_strike(p_strike)
{}
...
double PayoffPut::operator()(double p_spot) const
{
